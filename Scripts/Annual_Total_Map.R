...
##############################Legend############################################
###Read Data########################
Data<-rio::import(paste("Data/cum_95th_Perc_1983_2021_CHIRPS.csv",sep=""))

Raster_file<-rasterFromXYZ(Data[c("X","Y","cum")])
...
mybreaks <- c(0,50,150,200,300,600,1000,Inf)
...
breaklabel <- function(x){
  labels<-as.character(c(0,50,150,200,300,600,1000))
  #labels<- as.character(seq(1,5))
  labels[1:x]
}
################################################################################

Title<-toupper(paste("Rainfall average yearly totals over Kenya","\nRef: 1983-2022","\nData Source: ",Data_Source,sep=""))


l<-ggplot()+geom_contour_filled(data=Data, aes(x,y,z =cum),breaks= mybreaks, show.legend = TRUE) +
...
jpeg(filename = paste("Products/Annual_Total_Map_","_",Data_Source,".jpeg",sep=""),
     width = 12,
     height =14,
     units = "in",
     res=300)
